...
var M3 = class {
	...
	*/
	static async getInsets() {
		try {
			return await invoke("plugin:m3|insets");
		} catch {
			return false;
		}
	}
	...
	*/
	static async setBarColor(color = "system") {
		try {
			return "error" in await invoke("plugin:m3|bar_color", { color }) ? false : true;
		} catch {
			return false;
		}
	}
	...
	*/
	static async getColors(theme = "system") {
		try {
			let scheme = await invoke("plugin:m3|colors", { theme });
